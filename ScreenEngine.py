...
class GameSurface(ScreenHandle):
    ...
    def connect_engine(self, engine):
        # FIXME save engine and send it to next in chain - fixed
        self.engine = engine
        if self.successor:
            self.successor.connect_engine(self.engine)

    def draw_hero(self):
        self.engine.hero.draw(self, self.engine.hero)
    ...
    ##
        ...
        # draw next surface in chain - drawn
        super().draw(canvas)
...
class ProgressBar(ScreenHandle):
    ...
    def draw(self, canvas):
        coords = []
        step = self.engine.hero.sprite.get_width()
        for coord in self.engine.hero.position:
        ...
        pygame.draw.rect(self, colors["green"], (50, 70,
                                                 200 * self.engine.hero.exp / (100 * (2**(self.engine.hero.level - 1))),
                                                 30))

        font = pygame.font.SysFont("comicsansms", 20)
        self.blit(font.render(f'Hero at {coords}', True, colors["black"]),
                  (250, 0))
...
class InfoWindow(ScreenHandle):
    ...
    def draw(self, canvas):
        ...
        font = pygame.font.SysFont("comicsansms", 11)
        for i, text in enumerate(self.data):
            self.blit(font.render(text, True, colors["black"]),
                      (5, 20 + 18 * i))

            # FIXME draw next surface in chain - drawn
            super().draw(canvas)

    def connect_engine(self, engine):
        # FIXME set this class as observer save engine and send it to next in chain - fixed
...
class HelpWindow(ScreenHandle):
    ...
    # FIXME You can add some help information - Fixed
...
class MiniMap(ScreenHandle):
    ...
    def draw(self, canvas):
        ...
        if self.engine.show_minimap:
            pygame.draw.lines(self, (255, 0, 0, 255), True, [
                              (0, 0), (130, 0), (130, 100), (0, 100)], 3)
            coords = [self.engine.hero.position[0]/4.25, self.engine.hero.position[1]/4]
            pygame.draw.circle(self, colors["green"], (coords[0], coords[1]), 2)

            for obj in self.engine.objects:
                pygame.draw.circle(self, colors["white"], (obj.position[0]*step/4.25, obj.position[1]*step/4), 2)
