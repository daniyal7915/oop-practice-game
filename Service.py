...
def reload_game(engine, hero):
    global level_list
    size = hero.sprite.get_width()
    level_list_max = len(level_list) - 1
    engine.level += 1
    hero.position = [size, size]
    engine.objects = []
    generator = level_list[min(engine.level, level_list_max)]
    if generator == level_list[-1] or hero.hp <= 0:
        generator = level_list[-1]
        engine.hero.position = [size*5, size*2]
        if hero.hp <= 0:
            engine.notify(" ")
            engine.notify("---- GAME OVER ----")
            engine.notify("An enemy killed you")
        else:
            engine.notify(" ")
            engine.notify("---- GAME OVER ----")
            engine.notify("You reached the end")
            engine.notify("of life. Your gold ")
            engine.notify("will be inherited  ")
            engine.notify("by your descendants")
...
def drink_coffee(engine, hero):
    engine.hero = Objects.Coffee(hero)
    engine.notify("Coffee improved health")
...
class RandomMap(MapFactory):
    ...
    class Map:
        def __init__(self):
            self.Map = [[0 for _ in range(11)] for _ in range(8)]
            for i in range(11):
                for j in range(8):
                    if i == 0 or j == 0 or i == 10 or j == 7:
                        self.Map[j][i] = wall
                    else:
        ...
        def get_objects(self, _map):
            ...
            for obj_name in object_list_prob['objects']:
                ...
                for i in range(random.randint(prop['min-count'], prop['max-count'])):
                    coord = (random.randint(1, 10), random.randint(1, 7))
                    intersect = True
                    while intersect:
                        intersect = False
...
# FIXME  add classes for YAML !empty_map and !special_map{} - fixed
    ...
    class Objects:
        ...
        def get_objects(self, _map):
            ...
            for config_obj in self.config.keys():
                for obj_name in object_list_prob['enemies']:
                    if obj_name == config_obj:
                        prop = object_list_prob['enemies'][obj_name]
                        for i in range(self.config[config_obj]):
                            coord = (random.randint(1, 10), random.randint(1, 7))
                            intersect = True
                            while intersect:
                                intersect = False
